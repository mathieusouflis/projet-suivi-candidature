...
class ReminderController {
  getJobsToRemind = async (req, res) => {
    try {
      ...
      logger.error(`Erreur lors de la récupération des candidatures à relancer: ${error.message}`);
  ...
  setMeetingDate = async (req, res) => {
    try {
      ...
      logger.error(`Erreur lors de la définition de la date de rendez-vous: ${error.message}`);
      return res.status(500).json({
        success: false,
  ...
  setTestDate = async (req, res) => {
    try {
      ...
      logger.error(`Erreur lors de la définition de la date de test: ${error.message}`);
      return res.status(500).json({
        success: false,
